...
[ -z "$_AUTO" ] && _AUTO=$(~/.cursor/skills/gstack/bin/gstack-config get auto_upgrade 2>/dev/null || true)
...
```bash
if [ -d "$HOME/.cursor/skills/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.cursor/skills/gstack"
elif [ -d "$HOME/.gstack/repos/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.gstack/repos/gstack"
elif [ -d ".cursor/skills/gstack/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".cursor/skills/gstack"
elif [ -d ".agents/skills/gstack/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".agents/skills/gstack"
elif [ -d ".cursor/skills/gstack" ]; then
  INSTALL_TYPE="vendored"
  INSTALL_DIR=".cursor/skills/gstack"
elif [ -d "$HOME/.cursor/skills/gstack" ]; then
...
LOCAL_GSTACK=""
if [ -n "$_ROOT" ] && [ -d "$_ROOT/.cursor/skills/gstack" ]; then
  _RESOLVED_LOCAL=$(cd "$_ROOT/.cursor/skills/gstack" && pwd -P)
  _RESOLVED_PRIMARY=$(cd "$INSTALL_DIR" && pwd -P)
  if [ "$_RESOLVED_LOCAL" != "$_RESOLVED_PRIMARY" ]; then
    LOCAL_GSTACK="$_ROOT/.cursor/skills/gstack"
  fi
fi
_TEAM_MODE=$(~/.cursor/skills/gstack/bin/gstack-config get team_mode 2>/dev/null || echo "false")
...
```bash
cd "$_ROOT"
git rm -r --cached .cursor/skills/gstack/ 2>/dev/null || true
if ! grep -qF '.cursor/skills/gstack/' .gitignore 2>/dev/null; then
  echo '.cursor/skills/gstack/' >> .gitignore
fi
rm -rf "$LOCAL_GSTACK"
```
...
```
Tell user: "Also updated vendored copy at `$LOCAL_GSTACK` — commit `.cursor/skills/gstack/` when you're ready."
...
**If versions differ:** follow the Step 4.5 sync bash block above to update the local copy from the primary. Tell user: "Global v{PRIMARY_VER} is up to date. Updated local vendored copy from v{LOCAL_VER} → v{PRIMARY_VER}. Commit `.cursor/skills/gstack/` when you're ready."
