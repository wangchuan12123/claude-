...
[ -z "$_AUTO" ] && _AUTO=$(~/.slate/skills/gstack/bin/gstack-config get auto_upgrade 2>/dev/null || true)
...
```bash
~/.slate/skills/gstack/bin/gstack-config set auto_upgrade true
```
...
**If "Never ask again":**
```bash
~/.slate/skills/gstack/bin/gstack-config set update_check false
```
Tell user: "Update checks disabled. Run `~/.slate/skills/gstack/bin/gstack-config set update_check true` to re-enable."
Continue with the current skill.

### Step 2: Detect install type

```bash
if [ -d "$HOME/.slate/skills/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.slate/skills/gstack"
elif [ -d "$HOME/.gstack/repos/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.gstack/repos/gstack"
elif [ -d ".slate/skills/gstack/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".slate/skills/gstack"
elif [ -d ".agents/skills/gstack/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".agents/skills/gstack"
elif [ -d ".slate/skills/gstack" ]; then
  INSTALL_TYPE="vendored"
  INSTALL_DIR=".slate/skills/gstack"
elif [ -d "$HOME/.slate/skills/gstack" ]; then
...
LOCAL_GSTACK=""
if [ -n "$_ROOT" ] && [ -d "$_ROOT/.slate/skills/gstack" ]; then
  _RESOLVED_LOCAL=$(cd "$_ROOT/.slate/skills/gstack" && pwd -P)
  _RESOLVED_PRIMARY=$(cd "$INSTALL_DIR" && pwd -P)
  if [ "$_RESOLVED_LOCAL" != "$_RESOLVED_PRIMARY" ]; then
    LOCAL_GSTACK="$_ROOT/.slate/skills/gstack"
  fi
fi
_TEAM_MODE=$(~/.slate/skills/gstack/bin/gstack-config get team_mode 2>/dev/null || echo "false")
...
```bash
cd "$_ROOT"
git rm -r --cached .slate/skills/gstack/ 2>/dev/null || true
if ! grep -qF '.slate/skills/gstack/' .gitignore 2>/dev/null; then
  echo '.slate/skills/gstack/' >> .gitignore
fi
rm -rf "$LOCAL_GSTACK"
```
...
```
Tell user: "Also updated vendored copy at `$LOCAL_GSTACK` — commit `.slate/skills/gstack/` when you're ready."
...
```bash
~/.slate/skills/gstack/bin/gstack-update-check --force 2>/dev/null || \
.slate/skills/gstack/bin/gstack-update-check --force 2>/dev/null || true
```
...
**If versions differ:** follow the Step 4.5 sync bash block above to update the local copy from the primary. Tell user: "Global v{PRIMARY_VER} is up to date. Updated local vendored copy from v{LOCAL_VER} → v{PRIMARY_VER}. Commit `.slate/skills/gstack/` when you're ready."
